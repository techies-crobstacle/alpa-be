...
if (!txt.includes('sendFinanceOrderInvoiceEmail')) {
  txt = txt.replace('// Send Order Confirmation Email', newFn);

  const exportMatch1 = 'sendMonthlyGstReportEmail\r\n};';
  const exportMatch2 = 'sendMonthlyGstReportEmail\n};';

  if (txt.includes(exportMatch1)) {
    txt = txt.replace(exportMatch1, 'sendMonthlyGstReportEmail,\r\n  sendFinanceOrderInvoiceEmail\r\n};');
  } else if (txt.includes(exportMatch2)) {
    txt = txt.replace(exportMatch2, 'sendMonthlyGstReportEmail,\n  sendFinanceOrderInvoiceEmail\n};');
  }
